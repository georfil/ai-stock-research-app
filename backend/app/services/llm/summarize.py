...
def summarize_business_info(business_section: str) -> str:
    response = ask(text=business_section, system=BUSINESS_SUMMARY_PROMPT, model=get_config().budget_llm_model)
    return response
...
def retrieve_relevant_context(section_content: str, task: str) -> str:
    ...
    passages relevant to ``task`` rather than a summary. The extraction is a
